...
# Set paths:
inputdir = "/Users/katalinabobowik/Documents/UniMelb_PhD/Analysis/UniMelb_Sumba/Output/DE_Analysis/Yamagishi/"
...
# save both tables 
write.table(all.predicted.cellcounts, file=paste0(outputdir,"AllPredictedCellCounts_DeconCell_Yamagishi.txt"), sep="\t")
write.table(predicted.cellcounts, file=paste0(outputdir,"predictedCellCounts_DeconCell_Yamagishi.txt"), sep="\t")

# plot percentages of each cell type
pdf(paste0(outputdir,"DeconCell_RNASeqDeconvolution_Yamagishi.pdf"), height=10,width=19)
par(mar=c(14.1,4.1,10.1,2.1),xpd=T)
barplot(t(predicted.cellcounts), col=c("#ffd92f","#e78ac3","#fc8d62","#66c2a5","#8da0cb","#a6d854"), las=3, ylim=c(0,110))
legend(158,130,legend=colnames(predicted.cellcounts), col=c("#ffd92f","#e78ac3","#fc8d62","#66c2a5","#8da0cb","#a6d854"), pch=15, cex=0.8)
dev.off()

# get mean, median, and range for island by each cell type
