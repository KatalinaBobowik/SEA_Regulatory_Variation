...
gs.annots = buildIdx(entrezIDs=vDup$genes$entrezID, species="human",msigdb.gsets=c("c2", "c5"), go.part = TRUE)
...
gsa = egsea(voom.results=vDup, contrasts=contr.matrix, gs.annots=gs.annots, symbolsMap=symbolsMap, baseGSEAs=baseMethods, sort.by="p.adj", num.threads = 8, report = FALSE)

# Visualisation of results
# first make function for plotting
all.gsa.vis=function(db,pop,sorter){
    plotSummaryHeatmap(gsa, gs.label=db,hm.vals = "avg.logfc.dir",file.name=paste0(outputdir,"plotSummaryHeatmap_",db,".pdf"), sort.by="p.adj")
    plotSummary(gsa, gs.label = db, contrast = pop, file.name = paste0(outputdir,pop,db,sorter,"_plotSummary_.pdf"), x.cutoff=100, sort.by=sorter)
    plotMethods(gsa, gs.label = db, contrast = pop,file.name = paste0(outputdir,pop,db,"mds.pdf"))
    plotBars(gsa, gs.label = db, contrast = pop, file.name=paste0(outputdir,pop,db,sorter,"plotBars.pdf"), sort.by=sorter)
    write.table(topSets(gsa, contrast = pop,names.only=FALSE, number = Inf, verbose = FALSE, gs.label=db), file=paste0(outputdir,pop,db,"_topSets.txt"))
}

# now plot for all pops
for (pop in colnames(voomDupEfit)){
...
for (pop in colnames(voomDupEfit)){
    for (label in c("c5BP", "c5CC")){
        plotGOGraph(gsa, gs.label=label, contrast = pop, file.name=paste0(outputdir,pop,label,"_GOGraph.pdf"))
    }
}
...
for (pop in colnames(voomDupEfit)){
    plotPathway(gsa, gene.set = "Malaria",contrast = pop, gs.label = "kegg",file.name = paste0(outputdir,pop,"_pathwayMalaria.pdf"))
    plotHeatmap(gsa, gene.set="Malaria", gs.label="kegg",contrast = pop, file.name = paste0(outputdir,pop,"_kegg_Heatmap_Malaria.pdf"))   
}
