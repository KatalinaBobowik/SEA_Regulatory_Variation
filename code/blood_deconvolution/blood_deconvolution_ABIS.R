...
outputdir= "/Users/katalinabobowik/Documents/UniMelb_PhD/Analysis/UniMelb_Sumba/Output/DE_Analysis/123_combined/batchRemoval/ABIS/"
deconestimatedir= "/Users/katalinabobowik/Documents/UniMelb_PhD/Analysis/UniMelb_Sumba/Output/DE_Analysis/123_combined/batchRemoval/"
refdir= "/Users/katalinabobowik/Documents/UniMelb_PhD/Analysis/UniMelb_Sumba/ReferenceFiles/indoRNA_SequencingFiles/"
...
write.table(round(as.data.frame(cpm[,1:50]),2), file=paste0(outputdir,"cpm_Samples1to50.txt"), sep="\t")

# Now repeat the same thing for the rest of the samples
write.table(round(as.data.frame(cpm[,51:100]),2), file=paste0(outputdir,"cpm_Samples51to100.txt"), sep="\t")
write.table(round(as.data.frame(cpm[,101:123]),2), file=paste0(outputdir,"cpm_Samples101to123.txt"), sep="\t")
...
ABIS.decon1.50=read.table(paste0(outputdir,"cpm_Samples1to50.txt_deconvolution.txt"), header=T, row.names=1)
ABIS.decon51.100=read.table(paste0(outputdir,"cpm_Samples51to100.txt_deconvolution.txt"), header=T, row.names=1)
ABIS.decon101.123=read.table(paste0(outputdir,"cpm_Samples101to123.txt_deconvolution.txt"), header=T, row.names=1)
...
decon=read.table(paste0(deconestimatedir,"predictedCellCounts_DeconCell.txt"))
...
pdf(paste0(outputdir,"correlation_deconvolution_DeconCellvsABIS.pdf"))
...
norm.meth.decon=read.table(paste0(refdir,"indonesian_cell_counts_rough_estimate_new.txt"), header=T)
...
# now test correlation
pdf(paste0(outputdir,"correlation_deconvolution_ABISvsMeth_noLogTransform.pdf"))
# sapply(1:6, function(x) plot(RNA.decon[,x],norm.meth.decon[,x], main=paste(colnames(norm.meth.decon)[x],cor(RNA.decon[,x],norm.meth.decon[,x],method="pearson"),sep="\n")))
par(mfrow=c(2,3))
for (x in 1:6){
...
# save expression data. Unfortunately, saving the whole df with all decimal points is too large so we have to save it in chunks (with reduced decimal points)
write.table(round(as.data.frame(tpm[,1:39]),2), file=paste0(outputdir,"tpm_Samples1to39.txt"_, sep="\t")
# Now repeat the same thing for the rest of the samples
write.table(round(as.data.frame(tpm[,40:79]),2), file=paste0(outputdir,"tpm_Samples40to79.txt"), sep="\t")
write.table(round(as.data.frame(tpm[,80:119]),2), file=paste0(outputdir,"tpm_Samples80to119.txt"), sep="\t")
write.table(round(as.data.frame(tpm[,120:123]),2), file=paste0(outputdir,"tpm_Samples120to123.txt"), sep="\t")
...
# read in cpm_Samples1to50.txt from launched shiny app and download
# note: got weird error message: Warning in rlm.default(as.matrix(sigRNAseq[genes, ]), x) : 'rlm' failed to converge in 20 steps
ABIS.decon1.39=read.table(paste0(outputdir,"tpm_Samples1to39.txt_deconvolution.txt"), header=T, row.names=1)
ABIS.decon40.79=read.table(paste0(outputdir,"tpm_Samples40to79.txt_deconvolution.txt"), header=T, row.names=1)
ABIS.decon80.119=read.table(paste0(outputdir,"tpm_Samples80to119.txt_deconvolution.txt"), header=T, row.names=1)
ABIS.decon120.123=read.table(paste0(outputdir,"tpm_Samples120to123.txt_deconvolution.txt"), header=T, row.names=1)
...
# barplot
pdf(paste0(outputdir,"ABIS_RNASeqDeconvolution_TPM.pdf"), height=10,width=19)
par(mar=c(14.1,4.1,10.1,2.1),xpd=T)
barplot(t(predicted.cellcounts.scaled), las=3, col=c("#ffd92f","#e78ac3","#fc8d62","#66c2a5","#8da0cb","#a6d854"))
legend(123,130,pch=15, cex=0.8,legend=rownames(t(predicted.cellcounts.scaled)), col=c("#ffd92f","#e78ac3","#fc8d62","#66c2a5","#8da0cb","#a6d854"))
dev.off()
...
# now test correlation
pdf(paste0(outputdir,"correlation_deconvolution_DeconCellvsABIS_TPM.pdf"))
# sapply(1:6, function(x) plot(RNA.decon[,x],norm.meth.decon[,x], main=paste(colnames(norm.meth.decon)[x],cor(RNA.decon[,x],norm.meth.decon[,x],method="pearson"),sep="\n")))
par(mfrow=c(2,3))
for (x in 1:6){
	plot(ABIS[,x],decon[,x], main=paste(colnames(decon)[x],"\n r = ",round(cor(ABIS[,x],decon[,x],method="pearson"),digits=2),sep=""))
...
# now test correlation
pdf(paste0(outputdir,"correlation_deconvolution_ABISvsMeth_TPM.pdf"))
# sapply(1:6, function(x) plot(RNA.decon[,x],norm.meth.decon[,x], main=paste(colnames(norm.meth.decon)[x],cor(RNA.decon[,x],norm.meth.decon[,x],method="pearson"),sep="\n")))
par(mfrow=c(2,3))
for (x in 1:6){
	plot(ABIS.decon[,x],norm.meth.decon[,x], main=paste(colnames(norm.meth.decon)[x],"\n r = ",round(cor(ABIS.decon[,x],norm.meth.decon[,x],method="pearson"),digits=2),sep=""))
